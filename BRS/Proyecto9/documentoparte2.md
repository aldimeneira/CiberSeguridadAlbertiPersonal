Procdemos a descargar, instalar y habilitar apache2

![Instalacion Apache2](./img/instalacionyhabilitacionapache2.png)

Ahora, procedemos a instalar el SSL

![Instalacion SSL](./img/instalacionssl.png)

Creamos el certificado autofirmado de SSL

![Certificado SSL](./img/creacioncertificadoautofirmadossl.png)

Creamos una pagina de prueba

![PaginaPrueba](./img/creacionsitioweb.png)

![PaginaPrueba](./img/html.png)

Comprobamos el certificado de la página que hemos creado

![CertificadoPaginaPrueba](./img/certificadohtml.png)

Procedemos a redirigir al dominio creado a la ip que hemos creado en el servidor aws

![DNS](./img/dnsaldimeneira.png)

Creamos nuestro certificado propio 

![CertificadoPropio](./img/creacioncertificadopropio.png)

Una vez realizado esto, ya tenemos el dominio creado

![Dominio](./img/dominiocreado.png)

Nuestro certificado propio

![CertificadoPropio](./img/cerficiadopropio.png)



